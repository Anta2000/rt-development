...
def home(request):
    """ Page d'accueil """
    return render(request, 'home.html') # Renvoie le rendu du template 'home.html'
...
def liste_employes(request):
    try:
        structure = Infrastructure.objects.get(responsable=request.user.employe) # Récupère l'objet Infrastructure lié à l'utilisateur connecté
        employes = Employe.objects.filter(structure=structure)# Filtre les employés en fonction de la structure
...
def ajouter_employe(request):
    try:
        ...
        messages.error(request, "La structure n'existe pas.")
        return redirect('dashboard')# Redirige vers la page 'dashboard' en cas d'erreur
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('employe_list')#Redirige vers la liste des employés après l'ajout d'un employé
    else:
        form = EmployeForm()
    return render(request, 'ajouter_employe.html', {'form': form}) # Renvoie le rendu du template 'ajouter_employe.html' avec le formulaire
...
def supprimer_employe(request, employe_id):
    try:
        structure = Infrastructure.objects.get(responsable=request.user.employe) # Récupère l'objet Infrastructure lié à l'utilisateur connecté
        employe = get_object_or_404(Employe, id=employe_id, structure=structure)# Récupère l'employé correspondant à l'identifiant et à la structure
        user = employe.user
        employe.delete()
        user.delete()
